try {
	window.centaIOSdk = class CentaIOSdk {
		constructor(data) {
			this.baseUrlObj = {
				test: 'https://icentaiotracingapi.centaline.com.cn/api/monitor/v1.1/upload-log/app-list',
				prod: 'https://icentaiotracingapi.centaline.com.cn/api/monitor/v1.1/upload-log/app-list'
			};
			this.sendData = {
			};
		...
		setCommonProperty() {
			...
			this.sendData.userId = localStorage.getItem('monitorUserId') || "98ea098e-0548-c1ca-a0a7-08d814209efa";
			this.sendData.deptId = localStorage.getItem('monitorDeptId') || '';
		...
		customerInfo(centaIdObj) {
			if (this.getLocalStorage('monitorCentaId', true)) {
				let monitorCentaId = this.getLocalStorage('monitorCentaId', true);
				...
				this.setLocalStorage('monitorCentaId', monitorCentaId);
			} else {
